...
function read_position(){
    if (localStorage.getItem("grid-data") !== null) {
        ...
        for (var i = 0; i < positions.length; i++) {
            ...
            var widget = $(".gridster>ul>li[data-name="+position.name+"]");
...
function fill_restore_list(){
   ...
   if (hidden_widgets.exists()){
       hidden_widgets.each(function(i, widget) {
           var widget_name = $(widget).attr('data-name');
           restore_list.push( "<li class='widget-restore'><a href='#' onclick='show_widget(\""+widget_name+"\")'>" + $.humanize(widget_name.replace('_widget', '')) + "</a></li>" );
       });
   } else {
...
function show_widget(name){
    var gridster = $(".gridster>ul").gridster().data('gridster');
    var widget = $(".gridster>ul>li[data-name="+name+"]");
